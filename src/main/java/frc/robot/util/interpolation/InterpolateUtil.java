...
public class InterpolateUtil {
  ...
  public static double interpolate(InterpolationMap data, double x) {
    ...
    if (data.get(x) > -Double.MAX_VALUE) return data.get(x);
  ...
  /**
   * Linear interpolation between two 2D points. Finds the Y value between the two points given an X
   * value between the two points {@see https://theeducationlife.com/interpolation-formula/}
   *
  ...
  private static double linearInterpolation(
      double x1, double y1, double x2, double y2, double xBetween) {
